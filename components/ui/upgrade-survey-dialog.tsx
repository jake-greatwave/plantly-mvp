...
import type { UpgradeSurveyRequest, SurveyQ1Needs, SurveyQ2Price, SurveyQ3WTP } from '@/lib/types/survey.types'
...
export function UpgradeSurveyDialog({ open, onOpenChange, feature, onSuccess }: UpgradeSurveyDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Premium 기능을 3개월간 무료로 써보세요!</DialogTitle>
          <DialogDescription className="text-base pt-2">
            플랜틀리는 현재 베타 서비스 중입니다.
            <br />
            예상 가격 정책에 대한 간단한 의견(3문항)만 주시면,
            <br />
        ...
        <div className="space-y-6 py-4">
          <div className="space-y-3">
            ...
            <RadioGroup value={q1Needs} onValueChange={(value) => setQ1Needs(value as SurveyQ1Needs)}>
              {SURVEY_Q1_OPTIONS.map((option) => (
                <div key={option.value} className="flex items-center space-x-2">
                  <RadioGroupItem value={option.value} id={`q1-${option.value}`} />
                  <Label htmlFor={`q1-${option.value}`} className="font-normal cursor-pointer">
                    {option.label}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={q2Price} onValueChange={(value) => setQ2Price(value as SurveyQ2Price)}>
              {SURVEY_Q2_OPTIONS.map((option) => (
                <div key={option.value} className="flex items-center space-x-2">
                  <RadioGroupItem value={option.value} id={`q2-${option.value}`} />
                  <Label htmlFor={`q2-${option.value}`} className="font-normal cursor-pointer">
                    {option.label}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={q3WTP} onValueChange={(value) => setQ3WTP(value as SurveyQ3WTP)}>
              {SURVEY_Q3_OPTIONS.map((option) => (
                <div key={option.value} className="flex items-center space-x-2">
                  <RadioGroupItem value={option.value} id={`q3-${option.value}`} />
                  <Label htmlFor={`q3-${option.value}`} className="font-normal cursor-pointer">
                    {option.label}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-3">
            <Label htmlFor="q3-etc" className="text-base font-semibold">
              Q. 제조 솔루션 플랫폼 '플랜틀리'에 대해 자유로운 이야기를 부탁드립니다.
            </Label>
            <Textarea
              id="q3-etc"
              value={q3Etc}
